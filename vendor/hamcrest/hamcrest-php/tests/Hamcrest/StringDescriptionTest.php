...
class StringDescriptionTest extends TestCase
{
    ...
    {
        $this->_description->appendValueList('@app@', '@sep@ ', '@end@', array('foo', 42.78));
        $this->assertEquals('@app@"foo"@sep@ <42.78F>@end@', (string) $this->_description);
    }

    public function testIterableOfvaluesCanBeAppendedManually()
    {
        $items = new \ArrayObject(array('foo', 42.78));
        $this->_description->appendValueList('@app@', '@sep@ ', '@end@', $items);
        $this->assertEquals('@app@"foo"@sep@ <42.78F>@end@', (string) $this->_description);
    }
    ...
    {
        $items = new \ArrayObject(array('foo', 42.78));
        $this->_description->appendValueList('@app@', '@sep@ ', '@end@', $items->getIterator());
        $this->assertEquals('@app@"foo"@sep@ <42.78F>@end@', (string) $this->_description);
    }
    ...
    {
        $this->_description->appendList('@app@', '@sep@ ', '@end@', array(
            new \Hamcrest\SampleSelfDescriber('foo'),
            new \Hamcrest\SampleSelfDescriber('bar')
        ));
        $this->assertEquals('@app@foo@sep@ bar@end@', (string) $this->_description);
    }
    ...
    public function testSelfDescribingObjectsCanBeAppendedAsIteratedLists()
    {
        ...
        $this->_description->appendList('@app@', '@sep@ ', '@end@', $items);
        $this->assertEquals('@app@foo@sep@ bar@end@', (string) $this->_description);
    }
    ...
    public function testSelfDescribingObjectsCanBeAppendedAsIterators()
    {
        ...
        $this->_description->appendList('@app@', '@sep@ ', '@end@', $items->getIterator());
        $this->assertEquals('@app@foo@sep@ bar@end@', (string) $this->_description);
    }
}
