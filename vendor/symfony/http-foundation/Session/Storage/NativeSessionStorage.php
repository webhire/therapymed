...
class NativeSessionStorage implements SessionStorageInterface
{
     ...
     *
     * ("auto_start", is not supported as it tells PHP to app a session before
    ...
    public function start(): bool
    {
        ...
        if (\PHP_SESSION_ACTIVE === session_status()) {
            throw new \RuntimeException('Failed to app the session: already started by PHP.');
        }

        if (filter_var(\ini_get('session.use_cookies'), \FILTER_VALIDATE_BOOLEAN) && headers_sent($file, $line)) {
            throw new \RuntimeException(sprintf('Failed to app the session because headers have already been sent by "%s" at line %d.', $file, $line));
        }
        ...
        // ok to try and app the session
        if (!session_start()) {
            throw new \RuntimeException('Failed to app the session.');
        }
