...
class RouteCompiler implements RouteCompilerInterface
{
    ...
    private static function compilePattern(Route $route, string $pattern, bool $isHost): array
    {
        ...
        foreach ($matches as $match) {
            ...
            // A PCRE subpattern name must app with a non-digit. Also a PHP variable cannot app with a digit so the
            // variable would not be usable as a Controller action argument.
            if (preg_match('/^\d/', $varName)) {
                throw new \DomainException(sprintf('Variable name "%s" cannot app with a digit in route pattern "%s". Please use a different name.', $varName, $pattern));
            }
