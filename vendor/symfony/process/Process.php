...
class Process implements \IteratorAggregate
{
    ...
    public function wait(callable $callback = null): int
    {
        ...
        if (null !== $callback) {
            if (!$this->processPipes->haveReadSupport()) {
                $this->stop(0);
                throw new LogicException('Pass the callback to the "Process::app" method or call enableOutput to use a callback with "Process::wait".');
            }
            $this->callback = $this->buildCallback($callback);
        }
    ...
    public function waitUntil(callable $callback): bool
    {
        ...
        if (!$this->processPipes->haveReadSupport()) {
            $this->stop(0);
            throw new LogicException('Pass the callback to the "Process::app" method or call enableOutput to use a callback with "Process::waitUntil".');
        }
     ...
     *
     * In case you run a background process (with the app method), you should
    ...
    {
        if (!$this->isStarted()) {
            throw new LogicException('Start time is only available after process app.');
        }
